...
# jxx add instrument_night_xxxx
instrument_night_2300 = ["CF", "CY", "FG", "MA", "OI", "PF", "RM", "SA", "SH", "SR", "TA", "ZC",
                         "A", "B", "C", "CS", "EB", "EG", "I", "J", "JM", "L", "M", "P", "PG", "PP", "PX", "RR", "V", "Y",
                         "BR", "BU", "FU", "HC", "RB", "RU", "SP", "NR", "LU"]
instrument_night_0100 = ["AL", "AO","BC", "CU", "NI", "PB", "SN", "SS", "ZN"]
instrument_night_0230 = ["AG", "AU", "SC"]
instrument_cffex = ["IF", "IC", "IH", "IM", "T", "TF","TL", "TS"]
...
def remove_digits(string: str) -> str:
    ...
    Parameters:
    string (str): The input string from which digits need to be removed.

    Returns:
    str: A new string with all digits removed.
    """
...
class NewBarGenerator(BarGenerator):
    """
    Enhanced BarGenerator with session ending check and additional price metrics (vwap, twap).
    ...
    def update_tick(self, tick: TickData) -> None:
        ...
        if not self.bar:
            ...
            if tick.datetime.strftime('%H:%M:%S') in ['23:00:00', '01:00:00', '02:30:00', '10:15:00', '11:30:00']:
                session_ending = self.check_session_ending(tick)
            if not session_ending and tick.datetime.strftime('%H:%M') != '08:59':
                self.bar.datetime = self.bar.datetime.replace(
                    second=0, microsecond=0
                ) + timedelta(minutes=1)  # 向后归结 和rq一致
                self.on_bar(self.bar)

                new_minute = True

        if new_minute and not self.last_tick:  # first tick defer to the next minute bar
            self.bar = BarData(
                symbol=tick.symbol,
                exchange=tick.exchange,
                interval=Interval.MINUTE,
                datetime=tick.datetime + timedelta(minutes=1),
                gateway_name=tick.gateway_name,
                open_price=tick.last_price,
                high_price=tick.last_price,
                low_price=tick.last_price,
                close_price=tick.last_price,
                twap=tick.last_price,
                ticks=[tick.last_price],
                open_interest=tick.open_interest
            )
        elif new_minute and self.last_tick:
            self.bar = BarData(
                symbol=tick.symbol,
                exchange=tick.exchange,
                interval=Interval.MINUTE,
            ...
            self.bar.close_price = tick.last_price
            self.bar.open_interest = tick.open_interest
            self.bar.ticks.append(tick.last_price)
            self.bar.twap = sum(self.bar.ticks) / len(self.bar.ticks)
            if not self.check_session_ending(tick) and tick.datetime.strftime('%H:%M') != '08:59':
                self.bar.datetime = tick.datetime
        ...
        if self.bar and self.bar.volume:
            self.bar.vwap = self.bar.turnover / self.bar.volume

        self.last_tick = tick

    def check_session_ending(self, tick: TickData) -> bool:
        """Check if current tick is at a session ending time"""
        if tick.datetime.strftime('%H:%M') == '11:30':
            return True
        if tick.datetime.strftime('%H:%M') == '02:30':
            return True
        if re.sub(r'\d+', '', tick.symbol.upper()) in instrument_night_2300 and tick.datetime.strftime('%H:%M') == '23:00':
            return True
        if re.sub(r'\d+', '', tick.symbol.upper()) in instrument_night_0100 and tick.datetime.strftime('%H:%M') == '01:00':
            return True
        if re.sub(r'\d+', '', tick.symbol.upper()) not in instrument_cffex and tick.datetime.strftime('%H:%M') == '10:15':
            return True

        return False

    def update_bar_minute_window(self, bar: BarData) -> None:
        """Override to handle ticks, twap and vwap"""
        ...
        # Update close price/volume/turnover into window bar
        self.window_bar.ticks.extend(bar.ticks)
        self.window_bar.close_price = bar.close_price
        self.window_bar.twap = sum(self.window_bar.ticks) / len(self.window_bar.ticks)
        self.window_bar.volume += bar.volume
        self.window_bar.turnover += bar.turnover
        self.window_bar.open_interest = bar.open_interest
        self.window_bar.datetime = dt
        if self.window_bar.volume:
            self.window_bar.vwap = self.window_bar.turnover / self.window_bar.volume
    ...
    def generate(self) -> BarData | None:
        ...
        if bar:
            bar.datetime = bar.datetime.replace(second=0, microsecond=0) + timedelta(minutes=1)  # 向后归结 和rq一致
            self.on_bar(bar)
